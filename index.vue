...
<div>
  <table
    :style="{'font-size': tableStyles.fontSize}"
    :class="{
      'table-bordered is-bordered': isBordered,
      'table-striped is-striped': isStriped
    }"
    class="table is-narrow table-sm"
    style="margin-bottom:0;border-bottom:0;width: calc(100%)">
    <thead :class="headerClass">
      <slot name="table-header"></slot>
    </thead>
  </table>
  <div style="overflow:overlay" :style="{height: tableHeight || ''}" class="fixed-header-table-container">
    <table  :style="{'font-size': tableStyles.fontSize}" :class="bodyClass" class="table is-narrow" style="margin-bottom:0;border-bottom:0;">
      ...
      <slot v-else name="table-body"></slot>-->
    </table>
  </div>
</div>
</template>
